...
# ── What makes this the audio app ─────────────────────────────────────────────

_BITRATES = ['320', '256', '192', '128', '64']

# The in-card format/bitrate selectors are smaller than the controls below, so
# they carry their own sizing rather than the shared QComboBox rule.
_COMBO_SS = f"""
    QComboBox {{
        background: {_SURFACE};
...
class DownloadWorker(BaseDownloadWorker):
    """Audio only: take the best stream and transcode it with ffmpeg."""
...
if __name__ == '__main__':
    ...
    def _check_updates():
        ...
        start_update_check(window, 'youtube-audio-downloader')
